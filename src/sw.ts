...
const ROOT_URL = new URL('./', serviceWorker.location.href);
const EXTRA_ASSETS = [
  './models/kaze.glb',
  './models/michi.glb',
  './models/raiden.glb',
...
async function precacheBuild(): Promise<void> {
...
serviceWorker.addEventListener('install', (event) => {
  event.waitUntil(precacheBuild().then(() => serviceWorker.skipWaiting()));
});
      ...
      .then(async (cached) => {
        ...
        try {
          ...
          if (event.request.mode === 'navigate') {
            const cache = await caches.open(CACHE_NAME);
            const offlineResponse = await cache.match(ROOT_URL, { ignoreVary: true });
